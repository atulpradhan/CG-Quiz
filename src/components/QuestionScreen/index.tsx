...
const QuestionScreen: FC = () => {
  ...
  useEffect(() => {
    ...
    setLimitedQuestions(shuffledQuestions)
  }, [questions])

  const currentQuestion = limitedQuestions[activeQuestion]
  const { question, type, choices, code, image, correctAnswers } = currentQuestion || {}
  ...
  const onClickNext = () => {
    ...
    if (activeQuestion !== limitedQuestions.length - 1) {
      setActiveQuestion((prev) => prev + 1)
    } else {
      // how long does it take to finish the quiz
      const timeTaken = quizDetails.totalTime - timer
  ...
  return (
    <PageCenter>
      ...
      <QuizContainer selectedAnswer={selectedAnswer.length > 0}>
        ...
        <ButtonWrapper>
          <Button
            text={activeQuestion === limitedQuestions.length - 1 ? 'Finish' : 'Next'}
            onClick={onClickNext}
            icon={<Next />}
            iconPosition="right"
            disabled={selectedAnswer.length === 0}
